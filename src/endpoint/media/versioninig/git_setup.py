...
class RepoManager:
    def __init__(self, repo_dir: str):
        ...
        if not os.path.isdir(repo_path) or not os.path.exists(
            ...
            try:
                ...
                subprocess.run(
                    ["git", "add", ".gitattributes"], cwd=repo_path, check=True
                )
    ...
    def copy_and_commit_file(self, source_file, relative_path, commit_message):
        if not os.path.isdir(self.repo_path) or not os.path.exists(
            os.path.join(self.repo_path, ".git")
        ):
            raise Exception("Invalid Repo")

        target_path = os.path.join(self.repo_path, os.path.dirname(relative_path))
        ...
        subprocess.run(
            ["git", "lfs", "track", relative_path], cwd=self.repo_path, check=True
        )
        subprocess.run(["git", "add", relative_path], cwd=self.repo_path, check=True)
